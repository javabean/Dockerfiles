...
if [ "$1" = 'dnsmasq' -o "$1" = '/usr/sbin/dnsmasq' ]; then
	...
	CONSUL_TEMPLATE_OPTS="-template=/opt/dnsmasq/consul.json.ctmpl:/usr/local/etc/consul.d/dnsmasq.json"
	...
	#exec consul-template.sh -exec="$@ ${DNSMASQ_ARGS} ${DNSMASQ_EXTRA_OPTS}" -exec-kill-timeout="5s"
